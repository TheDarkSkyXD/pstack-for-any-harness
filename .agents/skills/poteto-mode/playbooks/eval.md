...
6. **Verify the chain from transcripts, not self-report.** Use the active harness's workspace-scoped transcript or conversation-history interface. Never search another workspace. Look at which files each candidate actually opened. Grade chain-following from the files it really read plus the shape of the code, never from the candidate's own claims.
