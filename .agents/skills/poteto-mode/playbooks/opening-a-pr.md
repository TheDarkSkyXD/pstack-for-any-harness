...
- `## Verification`. State how you ran each check and its rigor. Name the real path, such as the active harness's browser, desktop, terminal, or service-control capability, or the targeted tests. State the outcome of each check, not only the command name.
...
**Readiness.** Open every PR ready, never as a draft. If the PR creation tool has a draft option, set it to false. If a PR still opens as a draft, run the host's ready command, such as `gh pr ready <number>`. Run `gh pr view <number>` before you refer to PR status.

**Babysit.** Opening a PR does not start a babysit. Post the URL and keep building. Finish the phase or stack first. Run a separate babysit pass only when the user asks for one after the whole stack exists. A babysit for each new PR stalls the build and spends checks on commits that later waves restart. Push back when feedback drifts from intent.

A subagent that opens a PR runs `interrogate`, the available code-cleanup pass, and `.agents/skills/no-comments/SKILL.md`. It returns the URL and does not babysit. Return to the parent.
