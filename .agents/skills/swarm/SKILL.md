...
4. Pick the worker model from `swarm workers` in `.agents/pstack-models.md` when present. Otherwise use `grok-4.7-xhigh-fast`. For a model race, name each arm's model up front.
