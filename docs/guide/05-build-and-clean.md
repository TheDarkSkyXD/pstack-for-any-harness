...
Each of these routes to its playbook ([Bug fix](../../.agents/skills/poteto-mode/playbooks/bug-fix.md), [Feature](../../.agents/skills/poteto-mode/playbooks/feature.md), [Refactoring](../../.agents/skills/poteto-mode/playbooks/refactoring.md), [Perf issue](../../.agents/skills/poteto-mode/playbooks/perf-issue.md)), and the playbook supplies the steps you didn't type: reproduce before fixing, name the data shape before implementing, pin behavior before restructuring, profile before optimizing.

For sustained improvement of one number, there's the [Hillclimb playbook](../../.agents/skills/poteto-mode/playbooks/hillclimb.md). Give it the metric, a target, and a floor on attempts, and it loops one hypothesis at a time with a frozen measurement harness. It keeps wins and reverts everything else.
...
In context, that's enough. [`/tdd`](../../.agents/skills/tdd/SKILL.md) writes the smallest test that fails for the intended reason, then the fix, then reruns the test. If a test would need broad harness setup or brittle mocks, the skill says so and uses the closest executable check instead. Don't force a test where a real command is stronger evidence.

## Let the TypeScript rules load themselves

[`typescript-best-practices`](../../.agents/skills/typescript-best-practices/SKILL.md) has no slash command in your workflow. It loads whenever the agent touches a `.ts` or `.tsx` file and turns the type-system principles into concrete rules: discriminated unions, `unknown` at boundaries, exhaustive variants, schema-derived types.

## Clean before you commit

The [Opening a PR playbook](../../.agents/skills/poteto-mode/playbooks/opening-a-pr.md) uses the active harness's code-cleanup capability when available and applies [`/unslop`](../../.agents/skills/unslop/SKILL.md) to the PR description and commit bodies. Without a cleanup skill, it inspects the diff directly for narrating comments, unsupported guards, dead compatibility paths, and unrelated edits.
...
[`/no-comments`](../../.agents/skills/no-comments/SKILL.md) spawns [Comment Sicko](../../.agents/agents/comment-sicko.md), a read-only reviewer with a short keep list: license headers, doc comments on a public API, links that explain what code can't, behavior forced by an external dependency you can't reshape. Everything else goes. A surprise in your own code gets no such pass. The comment comes back as a refactor flag, and `/no-comments` fixes the flags it accepts at the root cause. When a comment claims a constraint, "do not remove", the skill offers to encode the claim as a type, test, or lint. Either way, the comment comes out.

The division of labor is worth keeping straight. The available code-cleanup pass cleans slop out of code, `/unslop` cleans it out of prose, and `/no-comments` hands comments to a reviewer who didn't write them.

**Pitfall:** cleanup is not optional polish. A diff with narrating comments and defensive dead weight reads as unfinished to reviewers, and the extra code is where the next bug hides. Ask for a cleanup pass before you commit, not after review calls it out.
